...
def system_startup(args=None, defs=None):
    ...
    if defs is not None:
        print(repr(defs))
    print(f'CPUs: {torch.get_num_threads()}, NPUs: {torch_npu.npu.device_count()} on {socket.gethostname()}.')
...
def automl_bridge(kettle, poison_delta, name, mode='poison-upload', dryrun=False):
    ...
    """Send data to autoML."""
    setup = dict(uid=name,
                 project_id='mimetic-science-301416"',
                 multilabel=False,
                 format='.png',
                 bucketname='paper_eps16_full',
                 display_name=name,
                 dataset_id=None,
                 model_id=None,
                 ntrial=1,
...
def set_random_seed(seed=233):
    ...
    torch_npu.npu.manual_seed(seed+2)
    # torch.cuda.manual_seed_all(seed + 3)
    torch_npu.npu.manual_seed(seed+3)
    np.random.seed(seed + 4)
    # torch.cuda.manual_seed_all(seed + 5)
    torch_npu.npu.manual_seed_all(seed+5)
    random.seed(seed + 6)
